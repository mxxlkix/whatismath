...
export default async function handler(request, response) {
  ...
  if (question.length > 4000) return response.status(413).json({ error: 'Please keep the question under 4,000 characters.' });

  const apiKey = process.env['GEMINI_API_KEY'];
  if (!apiKey) return response.status(500).json({ error: 'MathMate is not configured yet. Add GEMINI_API_KEY in Vercel project settings.' });
  ...
  try {
    ...
    if (!result.ok) throw new Error(data.error?.message || 'Gemini could not answer this question.');
    const answer = (data.candidates?.[0]?.content?.parts || []).map(part => part.text || '').join('').trim() || 'I could not find a math answer for that.';
    const safeAnswer = answer.replace(/[&<>]/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[character])).replace(/\n/g, '<br>');
